  ...
  return (
    <div className="fixed bottom-36 left-0 right-0 max-w-md mx-auto px-4 pointer-events-none z-10 touch-none">
      <div className="pointer-events-auto touch-auto">
        <div 
          className={`flex items-stretch justify-between gap-3 rounded-2xl backdrop-blur-xl bg-white/80 dark:bg-stone-900/80 p-4 shadow-2xl border border-white/50 dark:border-stone-700/50 transition-all duration-700 ease-in-out ${
            isTransitioning 
        ...
        >
          <div className="flex flex-col justify-between gap-1.5 flex-[2_2_0px]">
            <div className="flex flex-col gap-0.5">
              ...
              <p className="text-stone-500 dark:text-stone-400 text-xs font-normal leading-normal">
                {format(new Date(currentEvent.date), 'EEE, dd MMM', { locale: fr })} • {currentEvent.time}
              </p>
            </div>
            <Link 
          ...
          {events.map((_, index) => (
            <div
              ...
              className={`h-1 rounded-full transition-all duration-300 ${
                index === currentIndex
                  ? 'w-8 bg-primary'
                  : 'w-1 bg-stone-300 dark:bg-stone-600'
              }`}
            />
          ))}
        </div>
