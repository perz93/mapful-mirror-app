...
const BottomNavigation = ({ className = "" }: BottomNavigationProps) => {
  ...
  return (
    <>
      ...
      <div className={`fixed bottom-0 left-0 right-0 max-w-md mx-auto flex-shrink-0 z-40 ${className}`}>
        {/* Safe area background fill */}
        <div className="fixed bottom-0 left-0 right-0 bg-[#e8e4d8]" style={{ height: 'env(safe-area-inset-bottom, 0px)' }} />
        <div className="mx-3 h-[68px] rounded-xl backdrop-blur-xl bg-white/80 dark:bg-stone-900/80 shadow-2xl border border-stone-200/50 dark:border-stone-700/50 overflow-hidden" style={{ marginBottom: 'calc(env(safe-area-inset-bottom, 4px) + 2px)' }}>
